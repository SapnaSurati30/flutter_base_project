...
Future<bool> setValue(String key, dynamic value, {bool print = true}) async {
  ...
  if (value is String) {
    return await sharedPreferences.setString(key, value.validateString());
  } else if (value is int) {
    return await sharedPreferences.setInt(key, value.validateInt());
  } else if (value is bool) {
    return await sharedPreferences.setBool(key, value.validateBool());
  } else if (value is double) {
    return await sharedPreferences.setDouble(key, value.validateDouble());
...
Map<String, dynamic> getJSONAsync(String key, {Map<String, dynamic>? defaultValue}) {
  if (sharedPreferences.containsKey(key) && sharedPreferences.getString(key).validateString().isNotEmpty) {
