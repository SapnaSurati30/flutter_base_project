...
class _TextFormFieldWidgetState extends State<TextFormFieldWidget> {
  ...
  FormFieldValidator<String>? applyValidation() {
    if (widget.isValidationRequired.validateBool(value: true)) {
      if (widget.validator != null) {
        return widget.validator;
      } else if (widget.textFieldType == TextFieldType.EMAIL) {
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validateString(value: errorFiledMessage);
          if (!s.trim().validateEmail()) return widget.errorInvalidEmail.validateString(value: 'Email is invalid');
          return null;
        };
      } else if (widget.textFieldType == TextFieldType.PASSWORD) {
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validateString(value: errorFiledMessage);
          if (s.trim().length < passwordLengthCommon) return widget.errorMinimumPasswordLength.validateString(value: 'Minimum password length should be $passwordLengthCommon');
          return null;
        };
        ...
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validateString(value: errorFiledMessage);
          if (!s.validateURL()) {
            return widget.errorInvalidURL ?? 'Invalid Url';
          }
          return null;
        };
        ...
        return (s) {
          if (s!.trim().isEmpty) return widget.errorThisFieldRequired.validateString(value: errorFiledMessage);
          if (s.contains(' ')) {
            return widget.errorInvalidUsername.validateString(value: 'Username should not contain space');
          }
          return null;
        };
      } else {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      maxLines: widget.textFieldType == TextFieldType.ADDRESS ? null : widget.maxLines.validateInt(value: 1),
      minLines: widget.minLines.validateInt(value: 1),
      autofocus: widget.autoFocus ?? false,
      enabled: widget.enabled,
      onChanged: widget.onChanged,
      cursorColor: widget.cursorColor ?? Theme.of(context).textSelectionTheme.cursorColor,
      readOnly: widget.readOnly.validateBool(),
      maxLength: widget.maxLength,
      enableSuggestions: widget.enableSuggestions.validateBool(value: true),
      autofillHints: widget.autoFillHints,
      scrollPadding: widget.scrollPadding ?? EdgeInsets.all(20),
      cursorWidth: widget.cursorWidth.validateDouble(value: 2.0),
      cursorHeight: widget.cursorHeight,
      cursorRadius: Radius.circular(defaultTextFormFiledRadius),
      onTap: widget.onTap,
      ...
      expands: widget.expands.validateBool(),
      showCursor: widget.showCursor,
      selectionControls: widget.selectionControls,
      strutStyle: widget.strutStyle,
      obscuringCharacter: widget.obscuringCharacter.validateString(value: '•'),
